...
const CARD_IMAGES = {
  ...
  chennai: "https://images.unsplash.com/photo-1582407947304-fd86f28320be?w=900&q=80",
...
function Avatar({ initials, color, size = "sm" }: any) {
...
function PillFilterBar({ active, setActive }: any) {
  return (
    <div className="flex flex-wrap gap-2 py-3 px-8 border-b border-gray-100 bg-white sticky top-[63px] z-40">
      {TOPIC_FILTERS.map((t) => (
        <button
          ...
          className={`px-4 py-1.5 rounded-full text-sm font-medium border transition-all ${active === t.label
...
function ArticleCard({ article, size = "normal" }: any) {
    ...
    <div className="flex flex-col bg-white">
      <div className={`rounded-xl flex items-center justify-center ${imgH} mb-3 relative overflow-hidden`}>
        <img
          ...
          onError={(e) => { (e.target as any).style.display = 'none'; ((e.target as any).parentNode as any).style.background = '#e2e8f0'; }}
        />
        ...
        <ul className="space-y-0.5">
          {TOPIC_FILTERS.map((t) => (
            <li key={t.label}>
              <button
                onClick={() => setActive(t.label)}
                className={`w-full flex justify-between items-center px-2.5 py-1.5 rounded text-sm transition-colors ${active === t.label ? "bg-slate-800 text-white font-semibold" : "text-gray-700 hover:bg-gray-100"
                  }`}
              >
                <span>{t.label}</span>
...
function FeaturedHero({ onReadGuide }: any) {
  return (
    <div className="flex min-h-[320px]" style={{ maxWidth: "100%", overflow: "hidden" }}>
      {/* Left text */}
      <div className="bg-slate-700 w-1/2 p-10 flex flex-col justify-between relative overflow-hidden">
        ...
        <div className="flex items-center gap-4 mt-8 relative z-10">
          <button
            onClick={onReadGuide}
            className="bg-amber-500 hover:bg-amber-400 text-white font-semibold px-5 py-2.5 rounded-lg text-sm transition-colors"
          >
            Read the Guide →
          </button>
      ...
      <div className="w-1/2 relative overflow-hidden">
        ...
        <div className="absolute bottom-4 right-4 bg-white/90 text-slate-800 text-xs px-3 py-1.5 rounded-full flex items-center gap-1 shadow">
...
function BlogPage({ active, setActive, onReadGuide }: any) {
      ...
      <main className="flex-1 min-w-0 pl-8">
        ...
        <section className="mb-10">
          <div className="flex items-center justify-between mb-5">
            ...
            <a href="#" className="text-amber-500 text-sm font-medium hover:underline">View all →</a>
          </div>
          <div className="grid grid-cols-2 gap-6">
            <ArticleCard article={TOP_STORIES[0]} size="large" />
            <ArticleCard article={TOP_STORIES[1]} size="normal" />
          </div>
        </section>
        ...
        <section className="mb-10">
          ...
          <div className="grid grid-cols-4 gap-4">
            {LATEST_ARTICLES.map((a, i) => <ArticleCard key={i} article={a} />)}
          </div>
        </section>
          ...
          <div className="space-y-4">
            {QUICK_READS.map((a, i) => (
              <div key={i} className="flex items-center gap-4">
                <div className="w-20 h-14 rounded-lg shrink-0 overflow-hidden">
                  <img src={a.imgUrl} alt={a.title} className="w-full h-full object-cover" />
                </div>
                <div>
                  <div className="mb-1.5"><Tag label={a.tag} color={a.tagColor} /></div>
                  <p className="text-sm font-semibold text-slate-800 leading-snug">{a.title}</p>
                  <p className="text-xs text-gray-400 mt-0.5">{a.author} · {a.date} · {a.readTime}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <section className="mb-10">
          <div className="flex items-center justify-between mb-1">
            ...
            <a href="#" className="text-amber-500 text-sm font-medium hover:underline">Browse all articles →</a>
          </div>
          <p className="text-xs text-gray-400 mb-5">Selected based on what you've been reading</p>
          <div className="grid grid-cols-4 gap-4">
            {MIGHT_LIKE.map((a, i) => <ArticleCard key={i} article={a} />)}
          </div>
        </section>
        ...
        <div className="border-t border-gray-200 pt-4">
          <div className="flex items-center gap-4">
            <span className="text-xs font-bold text-gray-400 tracking-widest uppercase shrink-0">Also Read</span>
            <div
              ref={alsoReadRef}
            ...
            >
              {ALSO_READ.map((a, i) => (
                <div key={i} className="flex items-center gap-2 shrink-0 cursor-pointer group">
                  ...
                  <span className="text-xs text-gray-600 group-hover:text-slate-800 transition whitespace-nowrap">{a.label}</span>
                </div>
              ))}
            </div>
          </div>
...
function ArticlePage({ onBack }: any) {
  ...
  return (
    <div className="bg-white">
      ...
      <div className="max-w-5xl mx-auto px-8 pt-5 pb-2 flex items-center gap-2 text-xs text-gray-400">
        <button onClick={onBack} className="hover:text-amber-500 transition">City Guides</button>
        <span>›</span>
      ...
      <div className="max-w-5xl mx-auto px-8 pb-6">
        ...
        <h1 className="text-4xl font-extrabold text-slate-800 leading-tight mb-4 max-w-2xl" style={{ fontFamily: "Georgia, serif" }}>
          Chennai's Best Neighbourhoods for Renters in 2026
        </h1>
        <p className="text-gray-500 text-base max-w-xl leading-relaxed mb-5">
          A deep dive into the city's rental micro-markets — from student-friendly Adyar to the IT corridors of OMR — to help you find where you truly belong.
        </p>
          ...
          <div className="flex items-center justify-between flex-wrap gap-4">
            ...
            <div className="flex items-center gap-4 text-xs text-gray-400">
              ...
              <div className="flex items-center gap-1">
                <span>Share</span>
                <button className="w-5 h-5 bg-gray-100 rounded flex items-center justify-center text-gray-500 hover:bg-gray-200 text-xs">‹</button>
                <button className="w-5 h-5 bg-gray-100 rounded flex items-center justify-center text-gray-500 hover:bg-gray-200 text-xs">›</button>
              </div>
            </div>
          </div>
      ...
      <div className="max-w-5xl mx-auto px-8 pb-8 flex gap-8">
        ...
        {/* ── LEFT SHARE COLUMN (matches Figma design) ── */}
        <div className="hidden md:flex flex-col items-center gap-3 pt-1 w-10 shrink-0">
          <span className="text-[9px] font-bold tracking-widest text-gray-400 uppercase rotate-[-90deg] mb-4 whitespace-nowrap origin-center" style={{marginTop:"32px",marginBottom:"20px"}}>SHARE</span>
          {/* LinkedIn */}
          <button className="w-8 h-8 rounded-full border border-gray-200 flex items-center justify-center hover:border-blue-600 hover:bg-blue-50 transition group" title="Share on LinkedIn">
          ...
          {/* X / Twitter */}
          <button className="w-8 h-8 rounded-full border border-gray-200 flex items-center justify-center hover:border-slate-800 hover:bg-slate-50 transition group" title="Share on X">
          ...
          {/* Facebook */}
          <button className="w-8 h-8 rounded-full border border-gray-200 flex items-center justify-center hover:border-blue-500 hover:bg-blue-50 transition group" title="Share on Facebook">
            <svg className="w-4 h-4 text-gray-400 group-hover:text-blue-500" fill="currentColor" viewBox="0 0 24 24">
              <path d="M18 2h-3a5 5 0 00-5 5v3H7v4h3v8h4v-8h3l1-4h-4V7a1 1 0 011-1h3z"/>
            </svg>
          </button>
          {/* Copy link */}
          <button className="w-8 h-8 rounded-full border border-gray-200 flex items-center justify-center hover:border-amber-500 hover:bg-amber-50 transition group" title="Copy link">
            <svg className="w-4 h-4 text-gray-400 group-hover:text-amber-500" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
        ...
        <article className="flex-1 min-w-0">
          ...
          <p className="text-gray-700 text-base leading-relaxed mb-6">
            Chennai is one of India's most underrated rental markets. Unlike Delhi or Bangalore, the city offers a rare combination of affordability, cultural richness, and infrastructure quality — but only if you know exactly where to look. In this guide, we've analysed over 85,000 active listings on Rentit to bring you the most accurate neighbourhood-by-neighbourhood breakdown available.
          </p>

          {/* Section heading */}
          <h2 className="text-xl font-extrabold text-slate-800 mb-3">The South Zone: Where Affordability Meets Connectivity</h2>
          <p className="text-gray-600 text-sm leading-relaxed mb-6">
            The areas stretching from Adyar to OMR represent Chennai's most dynamic rental corridor. Technology parks have fundamentally transformed the fabric of this region over the past decade, driving demand for both premium and mid-range housing in equal measure.
          </p>
          ...
          <p className="text-gray-600 text-sm leading-relaxed mb-4">
            Adyar's proximity to IIT Madras, Anna University, and several reputable schools makes it perennially popular among academics, researchers, and families with school-going children. The neighbourhood strikes a careful balance between urban convenience and tree-lined calm.
          </p>
          ...
          <div className="bg-slate-800 rounded-xl p-6 mb-5 relative">
            ...
            <p className="text-white text-base italic leading-relaxed mt-5">
              Living in Adyar meant I could walk to the university, cycle to the beach, and still be at a decent restaurant by evening. No other neighbourhood gives you that trinity.
            </p>
          ...
          <div className="border-l-4 border-amber-400 bg-amber-50 rounded-r-lg p-4 mb-6">
            ...
            <p className="text-sm text-gray-600 leading-relaxed">
              Target side streets off 2nd Main Road and 6th Cross Street for better-priced flats with less negotiation pressure. Avoid main arterial roads if noise is a concern — traffic peaks around 8–10am daily.
            </p>
          </div>

          <p className="text-gray-600 text-sm leading-relaxed mb-8">
            Rental prices in Adyar typically range from ₹18,000 for a 1BHK to ₹45,000 for a 3BHK in a semi-furnished condition. Older independent houses often represent better value than apartments in new buildings, particularly if you're comfortable with aging fixtures.
          </p>

          {/* Adyar real image */}
          <div className="rounded-xl h-48 overflow-hidden mb-2">
            <img
              src={CARD_IMAGES.adyar}
              alt="Adyar neighbourhood"
              className="w-full h-full object-cover"
            />
          </div>
          <p className="text-xs text-gray-400 mb-8 text-center">A street intersection in Adyar · Rentit has 1,400+ listings in this micro-cluster</p>

          {/* OMR */}
          <h3 className="text-lg font-extrabold text-slate-800 mb-2">OMR — The IT Corridor</h3>
          <p className="text-gray-600 text-sm leading-relaxed mb-8">
            Old Mahabalipuram Road, commonly called OMR, stretches for over 45km and houses some of India's largest tech parks. For IT professionals, this is ground zero — but choosing the right stretch of OMR is absolutely critical to your quality of life.
          </p>
          ...
          <div className="border border-gray-200 rounded-xl p-5 mb-8">
            ...
            <p className="text-xs text-gray-500 leading-relaxed mt-4">
              Riya has been writing about India's rental markets for 7 years. She has personally lived in Chennai, Bangalore, and Hyderabad, and brings deeply researched, first-hand insight to every piece she writes. Her work is trusted by over 300,000 monthly readers on Rentit.
            </p>
          </div>
        </article>
              ...
              {TOC.map((item) => (
                <li key={item.section}>
                  <button
                    ...
                    className={`text-xs text-left w-full py-1 transition-colors ${activeSection === item.section ? "text-amber-500 font-semibold" : "text-gray-500 hover:text-slate-800"
                      }`}
                  >
                    {item.section}
                  </button>
                  {item.items.map((sub) => (
                    <p key={sub} className="text-[11px] text-gray-400 pl-3 py-0.5 hover:text-slate-700 cursor-pointer leading-tight">
                      {sub}
                    </p>
                  ))}
                </li>
      ...
      <div className="max-w-5xl mx-auto px-8 pb-8">
        <button
          onClick={onBack}
          className="flex items-center gap-2 text-sm text-gray-500 hover:text-slate-800 transition"
        >
          ← Back to City Guides
        </button>
      </div>
