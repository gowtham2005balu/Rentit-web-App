...
import { ArrowRight, ChevronDown, Check, Lightbulb, Headset, Search, Crosshair, ShieldCheck, ArrowUpDown, Snowflake, Dumbbell, Car, Waves, Zap, PawPrint, Wifi, Home, Trophy, Cctv, Flame, Droplets, CarFront, ShoppingBag, Camera, Video, X, Bell, MessageSquare, CircleHelp, Plus, ArrowLeft } from 'lucide-react';
import styles from '../add-property/addProperty.module.css';

export default function AddFlatmatePage() {
  ...
  const [listingPurpose, setListingPurpose] = useState('Flatmate');
  ...
  const [preferredTenant, setPreferredTenant] = useState('Female');
  ...
  const [expectedDeposit, setExpectedDeposit] = useState('120000');
  ...
  const [balconyRoom, setBalconyRoom] = useState(false);
  const [bathroomType, setBathroomType] = useState('Shared Bathroom');
  ...
  const validateStep = (step: number) => {
    switch (step) {
      case 1:
        ...
        return true;
      case 2:
        if (!apartmentType || !apartmentName || !bhkType || !builtUpArea || !floorNumber || !totalFloors || !roomType || !preferredTenant || !propertyAge) {
      ...
      case 3:
        ...
        return true;
      case 4:
        if (!monthlyRent || !expectedDeposit || !availableFrom || !furnishingType) {
          alert('Please fill in all pricing details and furnishing type.');
          return false;
        }
        return true;
      case 5:
  ...
  const handleSubmit = async () => {
    ...
    try {
      const payload = {
        ...
        apartmentType, apartmentName, bhkType, builtUpArea, floorNumber, totalFloors, roomType, preferredTenant, propertyAge, facing,
        locality, landmark, fullAddress,
        monthlyRent, expectedDeposit, monthlyMaintenance, maintenanceAmount, availableFrom, furnishingType, parkingAvailable, rentNegotiable, propertyDescription,
        attachedBathroom, acRoom, balconyRoom, bathroomType, nonVegAllowed, smokingAllowed, gym, gatedSecurity, whoWillShow, waterSupply, directionTips, selectedAmenities,
        availableDays, visitFrom, visitUntil, anytimeVisit, images,
      ...
      <div className={styles.mainContainer}>
        <div className={styles.subHeader}>
          <div>
            ...
            <p className={styles.pageSubtitle}>{currentStep === 8 ? 'Success' : `Step ${currentStep} of 8 — ${steps[currentStep - 1]?.name}`}</p>
          </div>
          ...
          <div className={styles.formCard} style={{ gridColumn: currentStep === 8 ? '1 / -1' : undefined }}>
            ...
            {currentStep === 2 && (
              <>
                ...
                <p style={{ color: '#64748B', fontSize: '14px', marginBottom: '24px' }}>Tell us about the property you're sharing with flatmates</p>

                <h3 style={{ fontSize: '16px', fontWeight: '600', color: '#0F172A', marginBottom: '16px' }}>Basic Property Info</h3>

                <div className={styles.inputGrid}>
                  <div className={styles.formGroup}>
                    <label className={styles.label}>Apartment Type <span className={styles.required}>*</span></label>
                    <div className={styles.inputWrapper}>
                      <select 
                        className={styles.input} 
                        style={{ appearance: 'none', cursor: 'pointer' }}
                  ...
                  <div className={styles.formGroup}>
                    <label className={styles.label}>Apartment Name <span className={styles.required}>*</span></label>
                    <div className={styles.inputWrapper}>
                      <input 
                        type="text" 
                        className={styles.input} 
                      ...
                      />
                      <ChevronDown size={16} className={styles.selectIcon} style={{ opacity: 0.5 }} />
                    </div>
                  </div>

                  <div className={styles.formGroup}>
                    <label className={styles.label}>BHK Type <span className={styles.required}>*</span></label>
                    <div className={styles.inputWrapper}>
                      <select 
                        className={styles.input} 
                        style={{ appearance: 'none', cursor: 'pointer' }}
                ...
                <div className={styles.inputGrid} style={{ marginTop: '24px' }}>
                  <div className={styles.formGroup}>
                    <label className={styles.label}>Room Type <span className={styles.required}>*</span></label>
                    <div className={styles.optionsGrid} style={{ gridTemplateColumns: '1fr 1fr' }}>
                      <div 
                        className={`${styles.optionBtn} ${roomType === 'Single Room' ? styles.optionActive : ''}`}
                        onClick={() => setRoomType('Single Room')}
                        style={{ padding: '10px' }}
                      >
                        Single Room
                      </div>
                      <div 
                        className={`${styles.optionBtn} ${roomType === 'Shared Room' ? styles.optionActive : ''}`}
                  ...
                  <div className={styles.formGroup}>
                    <label className={styles.label}>Preferred Tenant <span className={styles.required}>*</span></label>
                    <div className={styles.optionsGrid} style={{ gridTemplateColumns: 'repeat(3, 1fr)' }}>
                      <div 
                        className={`${styles.optionBtn} ${preferredTenant === 'Male' ? styles.optionActive : ''}`}
                        onClick={() => setPreferredTenant('Male')}
                        style={{ padding: '10px' }}
                      >
                        Male
                      </div>
                      <div 
                        className={`${styles.optionBtn} ${preferredTenant === 'Female' ? styles.optionActive : ''}`}
                        onClick={() => setPreferredTenant('Female')}
                        style={{ padding: '10px' }}
                      >
                        Female
                      </div>
                      <div 
                        className={`${styles.optionBtn} ${preferredTenant === 'Any' ? styles.optionActive : ''}`}
                        onClick={() => setPreferredTenant('Any')}
                        style={{ padding: '10px' }}
                      >
                        Any
                      </div>
            ...
            {currentStep === 3 && (
              <>
                <h2 className={styles.formTitle} style={{ fontSize: '18px', fontWeight: '600', color: '#0F172A', marginBottom: '24px' }}>Locality Details</h2>
                ...
                <div className={styles.formGroup}>
                  ...
                  <div className={styles.mapContainer}>
                    ...
                    <div className={styles.mapCenterPin}>
                      ...
                      <div style={{ marginTop: '8px', textAlign: 'center', backgroundColor: 'rgba(255, 255, 255, 0.9)', padding: '4px 8px', borderRadius: '4px', fontSize: '12px' }}>
                        <div style={{ fontWeight: '600', color: '#1E293B' }}>Anna Nagar, Chennai</div>
                        <div style={{ color: '#64748B', fontSize: '10px' }}>Click to adjust pin location</div>
                      </div>
                    </div>
                    {/* Map Background Placeholder */}
                    <div style={{ width: '100%', height: '100%', backgroundColor: '#E2E8F0', display: 'flex', flexDirection: 'column' }}>
                      <div style={{ display: 'flex', flex: 1 }}>
                        <div style={{ flex: 1, backgroundColor: '#E2E8F0', borderRight: '12px solid white' }}></div>
                        <div style={{ flex: 1, backgroundColor: '#E2E8F0' }}></div>
                      </div>
            ...
            {currentStep === 4 && (
              <>
                <h2 className={styles.sectionTitle} style={{ fontSize: '18px', fontWeight: '600', color: '#0F172A', marginBottom: '24px' }}>Pricing Details</h2>
                ...
                <div className={styles.inputGrid}>
                  <div className={styles.formGroup}>
                    ...
                    <div className={styles.currencyInputWrapper}>
                      ...
                      <input type="text" value={monthlyRent} onChange={e => setMonthlyRent(e.target.value)} className={styles.currencyInput} placeholder="0" />
                    </div>
                  </div>
                  <div className={styles.formGroup}>
                    ...
                    <div className={styles.currencyInputWrapper}>
                      <span className={styles.currencySymbol}>₹</span>
                      <input type="text" value={expectedDeposit} onChange={e => setExpectedDeposit(e.target.value)} className={styles.currencyInput} placeholder="0" />
                    </div>
                  </div>
                </div>
                ...
                <div className={styles.inputGrid}>
                  <div className={styles.formGroup}>
                    ...
                    <div className={styles.inputWrapper}>
                      <select className={styles.input} value={monthlyMaintenance} onChange={e => setMonthlyMaintenance(e.target.value)} style={{ appearance: 'none', cursor: 'pointer' }}>
                  ...
                  <div className={styles.formGroup}>
                    ...
                    <div className={styles.currencyInputWrapper}>
                      <span className={styles.currencySymbol}>₹</span>
                      <input type="text" value={maintenanceAmount} onChange={e => setMaintenanceAmount(e.target.value)} className={styles.currencyInput} placeholder="0" />
                    </div>
                  </div>
                </div>
                ...
                <div className={styles.inputGrid}>
                  ...
                  <div className={styles.formGroup}>
                    {/* Empty cell for grid alignment, just like the mockup layout implies if next fields are on a new line or not. Wait, the mockup shows Available From on the left, but nothing on the right next to it? Wait, let me check the mockup image closely. 
                    Yes, 'Available From' is on the left, and the right side of it is empty!
                    So this empty div works. */}
                  </div>
                </div>
                ...
                <div className={styles.inputGrid}>
                  <div className={styles.formGroup}>
                    ...
                    <div className={styles.inputWrapper}>
                      <select className={styles.input} value={furnishingType} onChange={e => setFurnishingType(e.target.value)} style={{ appearance: 'none', cursor: 'pointer' }}>
                        <option value="">Select</option>
                  ...
                  <div className={styles.formGroup}>
                    ...
                    <div className={styles.inputWrapper}>
                      <select className={styles.input} value={parkingAvailable} onChange={e => setParkingAvailable(e.target.value)} style={{ appearance: 'none', cursor: 'pointer' }}>
                        <option value="">Select</option>
            ...
            {currentStep === 5 && (
              <>
                <h2 className={styles.sectionTitle} style={{ fontSize: '18px', fontWeight: '600', color: '#0F172A', marginBottom: '24px' }}>Room Details</h2>
                
                <div className={styles.inputGrid} style={{ gridTemplateColumns: 'repeat(3, 1fr)' }}>
                  <div className={styles.formGroup}>
                    <label className={styles.label}>Attached Bathroom <span className={styles.required}>*</span></label>
                    <div className={styles.yesNoGroup}>
                      <button className={`${styles.yesNoBtn} ${!attachedBathroom ? styles.yesNoBtnActive : ''}`} onClick={() => setAttachedBathroom(false)}>No</button>
                      <button className={`${styles.yesNoBtn} ${attachedBathroom ? styles.yesNoBtnActive : ''}`} onClick={() => setAttachedBathroom(true)}>Yes</button>
                    </div>
                  </div>
                  <div className={styles.formGroup}>
                    <label className={styles.label}>AC Room <span className={styles.required}>*</span></label>
                    <div className={styles.yesNoGroup}>
                      <button className={`${styles.yesNoBtn} ${!acRoom ? styles.yesNoBtnActive : ''}`} onClick={() => setAcRoom(false)}>No</button>
                      <button className={`${styles.yesNoBtn} ${acRoom ? styles.yesNoBtnActive : ''}`} onClick={() => setAcRoom(true)}>Yes</button>
                    </div>
                  </div>
                  <div className={styles.formGroup}>
                    <label className={styles.label}>Balcony <span className={styles.required}>*</span></label>
                    <div className={styles.yesNoGroup}>
                      <button className={`${styles.yesNoBtn} ${!balconyRoom ? styles.yesNoBtnActive : ''}`} onClick={() => setBalconyRoom(false)}>No</button>
                      <button className={`${styles.yesNoBtn} ${balconyRoom ? styles.yesNoBtnActive : ''}`} onClick={() => setBalconyRoom(true)}>Yes</button>
                    </div>
                  </div>
                </div>
                ...
                <div className={styles.formGroup}>
                  ...
                  <div className={styles.toggleGroup} style={{ justifyContent: 'flex-start' }}>
                    {['Private Bathroom', 'Shared Bathroom'].map((type) => (
                      <button 
                        key={type}
                        className={`${styles.toggleBtn} ${bathroomType === type ? styles.active : ''}`}
                ...
                <div className={styles.inputGrid}>
                  ...
                  <div className={styles.formGroup}>
                    <label className={styles.label}>Smoking Allowed <span className={styles.required}>*</span></label>
                    <div className={styles.yesNoGroup}>
                      <button className={`${styles.yesNoBtn} ${!smokingAllowed ? styles.yesNoBtnActive : ''}`} onClick={() => setSmokingAllowed(false)}>No</button>
                      <button className={`${styles.yesNoBtn} ${smokingAllowed ? styles.yesNoBtnActive : ''}`} onClick={() => setSmokingAllowed(true)}>Yes</button>
                    </div>
                  </div>
                </div>
                ...
                <div className={styles.sectionSubtitle}>
                  Additional Details
                  <small style={{ marginLeft: '8px', color: '#94A3B8' }}>for maximum visibility</small>
                </div>
                ...
                <div className={styles.inputGrid}>
                  <div className={styles.formGroup}>
                    ...
                    <div className={styles.inputWrapper}>
                      <select className={styles.input} value={whoWillShow} onChange={e => setWhoWillShow(e.target.value)} style={{ appearance: 'none', cursor: 'pointer' }}>
                ...
                <div className={styles.inputGrid}>
                  <div className={styles.formGroup}>
                    ...
                    <div className={styles.inputWrapper}>
                      <select className={styles.input} value={waterSupply} onChange={e => setWaterSupply(e.target.value)} style={{ appearance: 'none', cursor: 'pointer' }}>
              ...
              <>
                <div className={styles.uploadCard}>
                  ...
                  <div 
                    ...
                    onClick={() => document.getElementById('flatmate-file-upload')?.click()}
                  ...
                  >
                    ...
                    <label className={styles.browseBtn} style={{ display: 'inline-block', cursor: 'pointer' }} onClick={(e) => e.stopPropagation()}>
                      Browse Files
                      <input id="flatmate-file-upload" type="file" multiple accept="image/*" onChange={handleImageUpload} style={{ display: 'none' }} />
                    </label>
                  </div>
                ...
                <div className={styles.uploadCard}>
                  <div 
                    ...
                    onClick={() => document.getElementById('flatmate-video-upload')?.click()}
                    style={{ cursor: 'pointer' }}
                  >
                    ...
                    <label className={styles.browseBtn} style={{ display: 'inline-block', cursor: 'pointer', marginTop: '16px' }} onClick={(e) => e.stopPropagation()}>
                      Browse Files
                      <input id="flatmate-video-upload" type="file" accept="video/*" onChange={handleVideoUpload} style={{ display: 'none' }} />
                    </label>
                  </div>
            ...
            {currentStep === 7 && (
              <div className={styles.fadeEnter}>
                <h2 className={styles.formTitle}>Visit Availability</h2>
          ...
          {currentStep !== 8 && (
            <div className={styles.rightSidebar}>
              ...
              {(currentStep === 5 || currentStep === 6) && (
                <div className={styles.tipsCard} style={{ backgroundColor: '#DCFCE7', borderColor: '#BBF7D0', marginBottom: '16px' }}>
                  <div className={styles.tipsTitle} style={{ color: '#166534' }}>
                    <Check size={16} />
                    9 amenities selected
                  </div>
                  <div style={{ color: '#166534', fontSize: '12px', lineHeight: '1.5' }}>
