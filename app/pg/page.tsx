...
export const metadata: Metadata = {
  title: "PG Accommodation in Chennai – RentIt",
  description: "Looking for a PG in Chennai? RentIt lists verified paying guest accommodations and hostels near IT parks, colleges, and city centers. Male, female & co-ed options.",
  robots: { index: true, follow: true },
  alternates: { canonical: "https://rentit.in/pgs" },
  openGraph: {
    title: "PG Accommodation in Chennai – RentIt",
    description: "Find verified PGs and hostels in Chennai near OMR, T. Nagar, Guindy and more.",
    url: "https://rentit.in/pgs",
    type: "website",
  }
};

export default async function PgList({ searchParams }: { searchParams: Promise<{ 
  propertyType?: string;
  roomType?: string;
  availability?: string;
  tenantPref?: string;
  ...
  try {
    ...
    if (allRows.length > 0) {
      liveProperties = allRows.filter((row: any) => 
        (row.type || '').toLowerCase().includes('pg') ||
        (row.propertyType || '').toLowerCase().includes('pg')
      ).map((row: any, index: number) => ({
        id: String(row._id || row.id),
        type: row.type || 'PG',
        title: row.title || 'Untitled PG',
  ...
  const filteredOwnerListed = filteredProperties.slice(0, 4);
  const filteredMetroListed = filteredProperties.slice(0, 10);
  const liveTrendingLocalities = Array.from(new Set(liveProperties.map(p => p.location))).slice(0, 5).map((loc, i) => ({
    id: `loc-${i}`,
    name: loc.split(',')[0],
    avgPrice: 'Avg ₹10,000/mo',
    imageUrl: 'https://images.unsplash.com/photo-1582510003544-4d00b7f74220?auto=format&fit=crop&q=80&w=300'
  }));
  return (
      ...
      <main className={styles.mainWrapper}>
        <div className={styles.layout}>
          ...
          <div className={styles.mainContent}>
            ...
            <div className={styles.pageHeader}>
              <div className={styles.breadcrumbs}>
                ...
                <Link href="/pg">Chennai</Link>
                <ChevronRight size={14} className={styles.breadcrumbIcon} />
                <Link href="/pg">PG & Hostels</Link>
                <ChevronRight size={14} className={styles.breadcrumbIcon} />
                <span className={styles.breadcrumbActive}>{locality ? locality.charAt(0).toUpperCase() + locality.slice(1) : 'Chennai'}</span>
              </div>
              
              <div className={styles.titleRow}>
                <h1 className={styles.pageTitle}>{filteredProperties.length.toLocaleString()} PGs in Chennai</h1>
            ...
            <HorizontalScrollSection properties={liveProperties} />
            ...
            <div className={styles.standardSection}>
              ...
              <div style={{ marginTop: '24px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
                  ...
                  <a href="/pg" style={{ color: '#D97706', fontSize: '13px', fontWeight: 600, textDecoration: 'none' }}>View all &rarr;</a>
                </div>
              ...
              <div style={{ marginTop: '32px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
                  ...
                  <a href="/pg" style={{ color: '#D97706', fontSize: '13px', fontWeight: 600, textDecoration: 'none' }}>Explore &rarr;</a>
                </div>
                <div className={styles.scrollContainer}>
          ...
          <RightSidebarWidgets recentItems={liveProperties.slice(0, 3).map(p => ({
            ...
            image: p.imageUrl || 'https://images.unsplash.com/photo-1545324418-cc1a3fa10c00?auto=format&fit=crop&q=80&w=300'
          }))} />
        </div>
      </main>
    </div>
